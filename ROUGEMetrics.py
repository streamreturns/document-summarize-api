# modified from https://github.com/seujung/KoBART-summarization/blob/2f7d7cd/rouge_metric.py
...
class ROUGEMetrics:
    ...
    def __init__(self, metrics=None, max_n=None, limit_length=True, length_limit=1000, length_limit_type='words', apply_average=True, apply_best=False, use_tokenizer=True, alpha=0.5, weight_factor=1.0):
        self.metrics = metrics[:] if metrics is not None else ROUGEMetrics.DEFAULT_METRICS
        for metric in self.metrics:
            if metric not in ROUGEMetrics.AVAILABLE_METRICS:
        ...
        if self.limit_length:
            if length_limit_type not in ROUGEMetrics.AVAILABLE_LENGTH_LIMIT_TYPES:
        ...
        if self.weight_factor <= 0:
            raise ValueError('ROUGE-W weight factor must be greater than 0.')
    ...
    def _get_word_ngrams_and_length(n, sentences):
        ...
        tokens = ROUGEMetrics._split_into_words(sentences)
        return ROUGEMetrics._get_ngrams(n, tokens), tokens, len(tokens) - (n - 1)
    ...
    def _compute_p_r_f_score(
        ...
        f_score = ROUGEMetrics._compute_f_score(precision, recall, alpha)
        return {'f': f_score, 'p': precision, 'r': recall}
    ...
    @staticmethod
    def _compute_n_grams(evaluated_sentences, reference_sentences, n):
        if len(evaluated_sentences) <= 0 or len(reference_sentences) <= 0:
            raise ValueError('Collections must contain at least 1 sentence.')

        evaluated_n_grams, _, evaluated_count = ROUGEMetrics._get_word_ngrams_and_length(
            n, evaluated_sentences
        )
        reference_n_grams, _, reference_count = ROUGEMetrics._get_word_ngrams_and_length(
            n, reference_sentences
        )
        # get the overlapping n-grams between evaluated and reference
        overlapping_n_grams = set(evaluated_n_grams.keys()).intersection(set(reference_n_grams.keys()))
        overlapping_count = 0
        for n_gram in overlapping_n_grams:
            overlapping_count += min(evaluated_n_grams[n_gram], reference_n_grams[n_gram])

        return evaluated_count, reference_count, overlapping_count

    @staticmethod
    def _compute_n_grams_lcs(evaluated_sentences, reference_sentences, weight_factor=1.0):
        def _lcs(x, y):
            m = len(x)
            n = len(y)
        ...
        evaluated_unigrams_dict, evaluated_count = ROUGEMetrics._get_unigrams(evaluated_sentences)
        reference_unigrams_dict, reference_count = ROUGEMetrics._get_unigrams(reference_sentences)
    ...
    def _get_scores_rouge_n(self, all_hypothesis, all_references):
        ...
        if self.apply_average or self.apply_best:
            scores = {metric: {stat: 0.0 for stat in ROUGEMetrics.STATS} for metric in metrics}
        else:
            scores = {
                metric: [{stat: [] for stat in ROUGEMetrics.STATS} for _ in range(len(all_hypothesis))]
                ...
                if self.apply_average:
                    ...
                    for reference in references:
                        (
                            ...
                            reference_count,
                            overlapping_ngrams,
                        ) = ROUGEMetrics._compute_n_grams(hypothesis, reference, n)
                    ...
                    score = ROUGEMetrics._compute_p_r_f_score(
                    ...
                    for stat in ROUGEMetrics.STATS:
        ...
        if (self.apply_average or self.apply_best) and len(all_hypothesis) > 1:
            for metric in metrics:
                for stat in ROUGEMetrics.STATS:
    ...
    def _get_scores_rouge_l_or_w(self, all_hypothesis, all_references, use_w=False):
        ...
        if self.apply_average or self.apply_best:
            scores = {metric: {stat: 0.0 for stat in ROUGEMetrics.STATS}}
        else:
            scores = {
                metric: [{stat: [] for stat in ROUGEMetrics.STATS} for _ in range(len(all_hypothesis))]
            }
            ...
            if self.apply_average:
                ...
                for reference_sentences in references_sentences:
                    (
                        ...
                        reference_count,
                        overlapping_ngrams,
                    ) = ROUGEMetrics._compute_n_grams_lcs(
    ...
    def _preprocess_summary_as_a_whole(self, summary):
        sentences = ROUGEMetrics.split_into_sentences(summary)
        ...
        summary = ROUGEMetrics.REMOVE_CHAR_PATTERN.sub(" ", summary.lower()).strip()


        tokens = self.tokenize_text(ROUGEMetrics.REMOVE_CHAR_PATTERN.sub(" ", summary))
    ...
    def _preprocess_summary_per_sentence(self, summary):
        ...
        final_sentences = []
        for sentence in sentences:
            sentence = ROUGEMetrics.REMOVE_CHAR_PATTERN.sub(" ", sentence.lower()).strip()


            tokens = self.tokenize_text(ROUGEMetrics.REMOVE_CHAR_PATTERN.sub(" ", sentence))
